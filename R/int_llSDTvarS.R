...
function(p, N_SA_RA,N_SA_RB, N_SB_RA, N_SB_RB, nRatings, nCond){
    ...
    c_RB <- c(p[nCond+nRatings], p[nCond+nRatings] + 
                cumsum(c(exp(p[(nCond+nRatings+1):(nCond + nRatings*2-1)]))), Inf)
    
    sd <- #sqrt(exp(p[nCond+2])^2 + (p[1:nCond]/2)^2  )
      sqrt(1 + (p[1:nCond]/2)^2 * exp(p[nCond + nRatings*2]))
    ...
    P_SBRB_SDT <- Vectorize(function(j,i) pnorm(q=c_RB[i+1], locB[j], sd[j]) - pnorm(q=c_RB[i], locB[j], sd[j]))
    P_SBRA_SDT <- Vectorize(function(j,i) pnorm(q=c_RA[i+1], locB[j], sd[j]) - pnorm(q=c_RA[i], locB[j], sd[j]))
    P_SARA_SDT <- Vectorize(function(j,i) pnorm(q=c_RA[i+1], locA[j], sd[j]) - pnorm(q=c_RA[i], locA[j], sd[j]))
    P_SARB_SDT <- Vectorize(function(j,i) pnorm(q=c_RB[i+1], locA[j], sd[j]) - pnorm(q=c_RB[i], locA[j], sd[j]))
